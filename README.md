...
### 4. 캐싱 (Redis, 정확 일치 + 시맨틱)
- Tavily 검색 결과: 검색어 기준 30분 TTL (무료 API 할당량 절약)
- RAG 답변: 2단계 캐시 — ①정확 일치(문자열 동일) → ②시맨틱(임베딩 코사인 유사도 ≥ 0.95, 문구가 달라도 의미가 비슷하면 히트). 각 5분 TTL
- Redis 장애 시에도 캐시만 건너뛰고 앱은 정상 동작 (페일세이프 설계)
- 상세 설계/실측 결과는 [시맨틱 캐싱](#시맨틱-캐싱) 섹션 참고
...
기존 Redis 캐시는 질문 문자열이 완전히 같을 때만 히트됩니다. 하지만 실제 사용자는 "파마리서치 직원수는?"과 "파마리서치 직원이 몇 명이야?"처럼 같은 의미를 다른 문장으로 묻습니다. 이 문제를 해결하기 위해 **임베딩 코사인 유사도 기반 2단계 캐시**를 구현했습니다.

```
질문 → [1차: 정확 일치 캐시] → 미스 → [2차: 시맨틱 캐시(코사인 유사도 ≥ 0.95)] → 미스 → LLM 실제 호출
```

1차에서 못 찾은 걸 2차가 찾으면, 그 결과를 1차 캐시에도 즉시 채워 넣어 다음번 동일 질문은 임베딩 계산 없이 더 빠르게 응답합니다. 조회 시 계산한 임베딩은 저장 단계까지 재사용해 API 호출을 절반으로 줄였습니다.

**실측 결과**: "파마리서치 직원수는?" → "파마리서치 직원이 몇 명이야?" 질의에서 코사인 유사도 0.9583으로 히트, 응답 시간이 약 17초(LLM 실제 호출)에서 1초 미만으로 단축됨.

**설계상 제외한 부분**: 스트리밍 엔드포인트(`/api/rag/ask/stream`)는 캐싱 대상에서 뺐습니다. 토큰이 조각조각 도착하는 스트림을 캐시했다가 그대로 재생하는 건 복잡도 대비 이득이 적다고 판단했습니다.

**주의할 점**: 임계값(0.95)은 임베딩 모델(`qwen3-embedding:0.6b`)의 실제 유사도 분포에 맞춰 실측으로 튜닝한 값입니다. 너무 낮추면 의미가 다른 질문도 히트해 오답이 재사용될 위험이 있고, 너무 높이면 정확 일치 캐시와 다를 게 없어집니다. LLM이 잘못된 답을 캐시해버릴 위험을 줄이기 위해 TTL을 5분으로 짧게 유지했습니다.

상세 설계, 실행/확인 명령어, 트러블슈팅은 [SETUP.md 22장](./SETUP.md#22-시맨틱-캐싱-임베딩-유사도-기반-캐시)을 참고하세요.
...
| `SemanticCacheService` 추가 후 앱 기동 실패 (`ObjectMapper` 빈을 찾을 수 없음) | 이 프로젝트에는 `ObjectMapper`가 자동 구성 빈으로 등록되어 있지 않은데, 새 클래스를 만들며 이미 있던 `OllamaService`의 패턴(직접 생성해서 사용)을 놓치고 빈 주입 방식으로 작성 | `OllamaService`와 동일하게 `static final ObjectMapper`를 클래스 안에서 직접 생성하도록 통일 |
...
- [x] **시맨틱 캐싱** — 임베딩 코사인 유사도 기반으로 문구가 달라도 의미가 비슷한 질문은 캐시 히트되도록 확장 (결과: [시맨틱 캐싱](#시맨틱-캐싱))
