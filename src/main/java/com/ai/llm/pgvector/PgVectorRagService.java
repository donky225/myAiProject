...
public class PgVectorRagService {
    ...
    public String ask(String question) {
        log.info("[pgvector] ask() 호출: \"{}\"", question);
    ...
    public Flux<String> askStream(String question) {
        log.info("[pgvector] askStream() 호출: \"{}\"", question);
        List<Document> relevantHits = getRelevantHits(question, ASK_FALLBACK_LIMIT);
        String prompt = buildStrictPrompt(question, relevantHits);
        return ollamaService.generateStreamReactive(prompt);
    }

    public RagAnswer askWithContext(String question) {
        log.info("[pgvector] askWithContext() 호출: \"{}\"", question);
        ...
        log.info("[pgvector] 답변 생성 완료 ({}ms, 컨텍스트 {}건)", elapsed, contexts.size());
    ...
    private List<Document> getRelevantHits(String question, int fallbackLimit) {
        ...
        log.debug("[pgvector] 벡터검색 후보 {}건 조회 (candidateCount={})", candidates.size(), candidateCount);
        ...
        if (rerankService.isEnabled()) {
            ...
            if (!reranked.isEmpty()) {
                ...
                List<Document> result = reranked.stream()
                        ...
                        .toList();
                log.info("[pgvector] 리랭킹 적용됨 → 최종 {}건 (임계값 {} 이상)", result.size(), RERANK_RELEVANCE_THRESHOLD);
                return result;
            }
            log.warn("[pgvector] 리랭크 결과 없음 → 코사인 유사도 폴백");
        ...
        List<Document> fallback = candidates.stream()
                ...
                .limit(fallbackLimit)
                .toList();
        log.debug("[pgvector] 코사인 유사도 필터 적용 → {}건 (임계값 {} 이상)", fallback.size(), RELEVANCE_THRESHOLD);
        return fallback;
    }
